...
class PricingProfileService {
  ...
  /// Returns every car currently connected to a pricing profile.
  /// The relationship is stored only on the car document as pricingProfileId.
  Future<List<Car>> getCarsForPricingProfile({
    required String tenantId,
    required String pricingProfileId,
  }) async {
    final tenant = _tenantId(tenantId);
    ...
    try {
      final snapshot = await _cars(tenant)
          .where('pricingProfileId', isEqualTo: profileId)
          .get();
      ...
      cars.sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
      return List.unmodifiable(cars);
    } on FirebaseException catch (e) {
      throw Exception('Unable to load connected cars: ${e.message ?? e.code}');
    }
  }
  ...
  Future<List<Car>> getCarsForPricingProfilePicker({
    required String tenantId,
    String search = '',
  }) async {
    final tenant = _tenantId(tenantId);
    ...
    try {
      ...
      throw Exception(
        'Unable to load cars for pricing connection: ${e.message ?? e.code}',
      );
    }
  }

  /// Connects multiple cars to one pricing profile.
  /// If a car belongs to another profile, this moves it to this profile.
  Future<void> connectCarsToPricingProfile({
    required String tenantId,
    required String pricingProfileId,
    ...
    final tenant = _tenantId(tenantId);
    final profileId = _profileId(pricingProfileId);
    final ids = carIds.map((id) => id.trim()).where((id) => id.isNotEmpty).toSet().toList();

    if (ids.isEmpty) return;

    try {
      final profileDoc = await _profiles(tenant).doc(profileId).get();
      ...
      for (var start = 0; start < ids.length; start += 450) {
        final end = (start + 450 < ids.length) ? start + 450 : ids.length;
        final batch = _firestore.batch();

        for (final carId in ids.sublist(start, end)) {
          batch.update(
            _cars(tenant).doc(carId),
            {
      ...
      throw Exception(
        'Unable to connect cars to pricing profile: ${e.message ?? e.code}',
      );
    }
  }

  /// Disconnects one car from its pricing profile.
  Future<void> disconnectCarFromPricingProfile({
    required String tenantId,
    required String carId,
  }) async {
    final tenant = _tenantId(tenantId);
    final id = _requiredId(carId, 'carId');

    try {
      final ref = _cars(tenant).doc(id);
      final snapshot = await ref.get();
      if (!snapshot.exists) throw Exception('Car not found.');
      ...
      throw Exception(
        'Unable to disconnect car from pricing profile: ${e.message ?? e.code}',
      );
    }
  }

  /// Disconnects multiple cars from their current pricing profiles.
  Future<void> disconnectCarsFromPricingProfile({
    required String tenantId,
    required List<String> carIds,
  }) async {
    final tenant = _tenantId(tenantId);
    ...
    try {
      ...
      throw Exception(
        'Unable to disconnect cars from pricing profile: ${e.message ?? e.code}',
      );
    }
  }
